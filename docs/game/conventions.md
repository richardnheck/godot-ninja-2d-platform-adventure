...
- **Object variants group together** where they share enough behaviour. Examples: `moving-platforms.md` covers Base/Vertical/Cave1H/Cave2H; `decorations.md` covers JapaneseLamp/SmallLantern/WaterSplash. Variants with distinct behaviour get their own file — e.g. `electricity-beam.md` (pulsing, timer-driven) and `megaman-electricity.md` (always-on) are separate. The unused experimental `Electricity.tscn` / `BlueFlame.tscn` scenes are not documented; nor are the unused test weapons (Pistol, MachineGun, GrenadeLauncher). The only shipping consumer of the `Gun.gd` base script is the **Cannon** (mis-spelled `Canon` in source — see [objects/hazards/cannon.md](objects/hazards/cannon.md)).
