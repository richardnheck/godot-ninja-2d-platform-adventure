...
signal pause_state_changed(status)
...
signal new_checkpoint(checkpoint, diff_previous)

export(String, "Idle", "Physics") var process_callback = "Idle" ## Determines weather the processing of elapsed time is calculated during process(_process) or during physics frames(_physics_process) Obs.: It only takes effect when the node first enters the SceneTree.
...
func get_time_dictionary_from_seconds(total_time_in_seconds: float) -> Dictionary:
...
func get_time_as_formatted_string(time_in_seconds: float, format: String) -> String:
	...
	while i < format.length():
		if format.substr(i, 1) == "{":
			...
			formatted_time += format.substr(i, 1)
			i += 1
